...
<template>
  <article class="device-page">
    <header class="os-bar">
      <RouterLink class="back" to="/projects">‹ Project library</RouterLink>
    ...
    <div class="app-window">
      <aside class="sidebar">
        ...
        <nav aria-label="诊断模块">
          <a class="active" href="#overview"><i>⌁</i>Overview</a>
      ...
      <main>
        <section id="overview" class="dashboard-head">
          ...
          <div class="health-score"><span>SYSTEM HEALTH</span><strong>READY</strong><i>6 / 6</i></div>
        </section>

        <section class="overview-grid">
          <figure class="device-preview">
            <div class="preview-toolbar"><span>LIVE DEVICE OVERVIEW</span><span>● RECENT SCAN</span></div>
          ...
          <div class="meta-panel">
            ...
            <div class="meter"><span>CAPABILITY COVERAGE</span><b><i></i></b><small>All core probes available</small></div>
          </div>
        </section>
        ...
        <section id="modules" v-if="project.modules?.length" class="module-console">
          <div class="section-label"><span>CAPABILITY MATRIX</span><small>{{ project.modules.length }} probes online</small></div>
        ...
        <section v-if="project.roadmap?.length" class="updates">
          ...
          <article v-for="phase in project.roadmap" :key="phase.label">
            <span>{{ phase.label }}</span><div><strong>{{ phase.title }}</strong><p>{{ phase.items.join(" · ") }}</p></div><i>INSTALLED</i>
          </article>
        </section>

        <nav class="pager" aria-label="项目翻页">
          <RouterLink v-if="previousProject" :to="`/projects/${previousProject.id}`"><span>← PREVIOUS DEVICE</span><strong>{{ previousProject.title }}</strong></RouterLink>
          <span v-else></span>
          <RouterLink v-if="nextProject" :to="`/projects/${nextProject.id}`"><span>NEXT DEVICE →</span><strong>{{ nextProject.title }}</strong></RouterLink>
        </nav>
      </main>
    </div>
  </article>

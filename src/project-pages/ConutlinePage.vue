...
<template>
    ...
    <main>
      <section class="command-hero">
        ...
        <div class="scan-output">
          ...
          <p>[<strong>DONE</strong>] report generated in 0.42s</p>
        </div>
        <div class="title-row">
          <div>
  ...
  └────────────────────┘</pre>
        ...
        <div class="hero-report">
          <figure><img :src="project.cover" :alt="project.coverAlt" /><figcaption>{{ project.visualLabel }}</figcaption></figure>
      ...
      <section class="file-table">
        ...
        <div class="table-head"><span>MODE</span><span>PATH / CAPABILITY</span><span>TYPE</span><span>LINES</span></div>
        ...
        <footer><span>{{ project.highlights.length }} entries</span><span>{{ project.highlights.length * 347 }} total lines</span></footer>
      </section>

      <section class="language-report">
        <div class="bars">
          <span>Static Analysis <i style="width:82%"></i><b>82%</b></span>
          <span>Gitignore Rules <i style="width:64%"></i><b>64%</b></span>
          <span>File Metrics <i style="width:51%"></i><b>51%</b></span>
          <span>Agent Context <i style="width:37%"></i><b>37%</b></span>
        </div>
        <div class="agent-prompt"><header>agent_context.txt <span>copy ↗</span></header><pre><code># Repository overview
...
status: ready_for_agent</code></pre></div>
      ...
      <nav class="pager" aria-label="项目翻页">
        <RouterLink v-if="previousProject" :to="`/projects/${previousProject.id}`"><span>cd ../previous</span><strong>← {{ previousProject.title }}</strong></RouterLink><span v-else></span>
        <RouterLink v-if="nextProject" :to="`/projects/${nextProject.id}`"><span>cd ../next</span><strong>{{ nextProject.title }} →</strong></RouterLink>
      </nav>
    </main>
  </article>
</template>

...
<template>
  <article class="studio-page">
    <header class="studio-bar">
      ...
      <div class="transport" aria-label="播放控制"><i>◀</i><b>▶</b><i>▶</i><time>00:00:00:00</time></div>
    ...
    <section class="hero">
      ...
      <figure class="video-monitor">
        ...
        <figcaption><span>CLIP_0124.MP4</span><b>PURE VISUAL</b><time>00:00:24:18</time></figcaption>
    ...
    <main>
      ...
      <nav class="pager" aria-label="项目翻页">
        <RouterLink v-if="previousProject" :to="`/projects/${previousProject.id}`"><span>◀ PREVIOUS CLIP</span><strong>{{ previousProject.title }}</strong></RouterLink><span v-else></span>
        <RouterLink v-if="nextProject" :to="`/projects/${nextProject.id}`"><span>NEXT CLIP ▶</span><strong>{{ nextProject.title }}</strong></RouterLink>
      </nav>
    </main>
  </article>
</template>

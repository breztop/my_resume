...
<template>
  <article class="network-page">
    <header class="masthead">
      <RouterLink to="/projects">INDEX / NETWORK SYSTEMS</RouterLink>
    ...
    <main>
      <section class="intro">
        <div class="intro-copy">
          ...
          <div class="line-count"><strong>{{ project.codeLines?.toLocaleString() || '20,000' }}</strong><span>LINES OF<br />NETWORKING CORE</span></div>
        </div>
      ...
      <section class="capabilities">
        <header><span>CAPABILITY MATRIX</span><h2>已交付的网络表面</h2><b>STATUS / IMPLEMENTED</b></header>
        <div class="matrix">
          <article v-for="(item,index) in project.highlights" :key="item">
            <span>{{ String(index + 1).padStart(2,'0') }}</span>
            <p>{{ item }}</p>
            <div><i /><em>{{ item.includes('规划') || item.includes('尚未') ? 'PLANNED' : 'STABLE' }}</em></div>
          </article>
        </div>
      </section>
    ...
    <footer>
      <div class="footer-title"><span>END OF DOCUMENT</span><strong>{{ project.title }} / {{ project.year }}</strong></div>
      <nav aria-label="项目翻页">
        <RouterLink v-if="previousProject" :to="`/projects/${previousProject.id}`"><span>← PREVIOUS DOCUMENT</span><b>{{ previousProject.title }}</b></RouterLink><i v-else />
        <RouterLink v-if="nextProject" :to="`/projects/${nextProject.id}`"><span>NEXT DOCUMENT →</span><b>{{ nextProject.title }}</b></RouterLink>
      </nav>
    </footer>
  </article>
</template>
...
<style scoped>
.network-page{--paper:#f0eee8;--ink:#171713;--orange:#ff5b2d;--lime:#cde757;--line:#b8b6ad;min-height:100vh;background:var(--paper);color:var(--ink);font-family:"IBM Plex Mono","SFMono-Regular",Consolas,monospace;background-image:linear-gradient(rgba(23,23,19,.045) 1px,transparent 1px),linear-gradient(90deg,rgba(23,23,19,.045) 1px,transparent 1px);background-size:24px 24px}.masthead{height:62px;border-bottom:1px solid var(--ink);display:flex;align-items:center;justify-content:space-between;padding:0 clamp(16px,4vw,60px);font-size:9px;letter-spacing:.14em}.masthead a{color:var(--ink);text-decoration:none}main{width:min(1420px,calc(100% - 48px));margin:auto;border-left:1px solid var(--line);border-right:1px solid var(--line)}.intro{min-height:720px;display:grid;grid-template-columns:1.35fr .65fr;border-bottom:1px solid var(--ink)}.intro-copy{padding:90px clamp(22px,5vw,80px);position:relative}.intro-copy>p,.intro-note>span,.topology header>span,.decisions article>span,.capabilities header>span,.releases header>span,.invariants header>span{font-size:9px;letter-spacing:.18em;color:var(--orange)}.intro h1{font:800 clamp(90px,15vw,230px)/.7 Inter,system-ui,sans-serif;letter-spacing:-.105em;margin:100px 0 65px}.intro h2{font:500 clamp(21px,2.3vw,34px)/1.3 Inter,system-ui,sans-serif;letter-spacing:-.03em;max-width:720px}.line-count{display:flex;align-items:center;gap:18px;margin-top:55px}.line-count strong{font:700 38px Inter,sans-serif}.line-count span{font-size:8px;line-height:1.4;color:#6b6a64}.intro-note{border-left:1px solid var(--line);padding:90px clamp(22px,3vw,48px);display:flex;flex-direction:column;justify-content:flex-end}.intro-note p{font:17px/1.85 Inter,"Noto Sans SC",sans-serif;margin:25px 0 0}

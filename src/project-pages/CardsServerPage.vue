...
<template>
  <article class="table-page">
    <header class="table-header">
      <RouterLink to="/projects">牌局目录</RouterLink>
      ...
      <section class="hand">
        ...
        <div class="cards">
          <article v-for="(item, index) in project.highlights" :key="item" :class="`card-${index + 1}`">
            ...
            <strong>{{ item }}</strong>
            <small>0{{ index + 1 }} / VALIDATED</small>
          </article>
        </div>
      </section>
    ...
    <footer>
      <p>END OF ROUND · SELECT NEXT TABLE</p>
      <nav aria-label="项目翻页">
        <RouterLink v-if="previousProject" :to="`/projects/${previousProject.id}`"><span>← 上一桌</span><b>{{ previousProject.title }}</b></RouterLink><i v-else />
        <RouterLink class="all" to="/projects">洗牌 / 全部项目</RouterLink>
        <RouterLink v-if="nextProject" :to="`/projects/${nextProject.id}`"><span>下一桌 →</span><b>{{ nextProject.title }}</b></RouterLink>
      </nav>
    </footer>
  </article>
</template>
